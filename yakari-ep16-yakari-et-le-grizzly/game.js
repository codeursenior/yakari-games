...
let theme = "autumn";            // autumn | winter | spring
let snowing = false;
let night = false;
let dawn = false;
...
let lastBump = 0;
let lightPos = null;             // 2e halo la nuit (le grizzly qu'on pousse)
...
function tileSolid(x, y) {
  const c = tileAt(x, y);
  if (c === "w") return !quest.inCanoe;   // le lac : seulement en canoë
...
function render(now) {
  ...
  drawList.push({
    y: player.y, fn: () => {
      ...
      else if (player.sprite === "canoe") drawCanoeRider(px, py, player.dir, t);
  ...
  // nuit : obscurité trouée autour du joueur (et du grizzly qu'on pousse)
  if (night) {
